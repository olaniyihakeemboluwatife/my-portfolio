...
function Header() {
  ...
  return (
    <header className="container mx-auto flex justify-between items-center py-4 max-width">
      {/* Logo and Menu Toggle */}
      <div className="flex justify-between items-center w-full md:w-auto py-2">
        <NavLink
          to="/"
          className="w-14 text-gradient text-2xl text-dark-heading dark:text-light-heading md:text-3xl xl:text-4xl xl:leading-tight font-bold"
        >
          HB
        </NavLink>
      ...
      <nav
        className={`${
          !isOpen ? "hidden" : null
        } text-center md:flex justify-between items-center w-full md:w-auto`}
      >
        <ul className="dark:text-light-content font-medium flex flex-col md:flex-row md:items-center md:space-x-5">
          <li>
            <NavLink
              to="/"
              onClick={toggleClass}
        ...
        {/* Social Links */}
        <ul className="flex justify-evenly items-center my-5 md:my-0 md:space-x-5 md:ml-5">
          <li>
            <a
              href={twitter}
              target="_blank"
